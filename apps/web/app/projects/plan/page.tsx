...
export default function PlanPage() {
  ...
  if (!brief) {
    return <main className="project-page"><nav><Link className="brand" href="/">AI VIDEO DIRECTOR</Link><AuthStatus /></nav><section className="empty"><h1>Nessun brief ancora.</h1><p>Prima crea un progetto, poi il Director potrà preparare il piano.</p><Link className="primary" href="/projects/new">Crea un progetto</Link></section></main>;
  }

  if (error) {
    return <main className="project-page"><nav><Link className="brand" href="/">AI VIDEO DIRECTOR</Link><AuthStatus /></nav><section className="empty"><h1>Qualcosa è andato storto.</h1><p>{error}</p><Link className="primary" href="/projects/new">Ricomincia</Link></section></main>;
  }

  if (!plan) {
    return <main className="project-page"><nav><Link className="brand" href="/">AI VIDEO DIRECTOR</Link><span className="nav-right"><span className="pill">STEP 02 / 04</span><AuthStatus /></span></nav><section className="plan-heading"><p className="eyebrow">DIRECTOR PLAN</p><h1>Sto preparando il piano…</h1><p>Il Director sta trasformando il brief in struttura, bibbia visiva e continuità.</p></section></main>;
  }
